...
void system_to_nuclear(System &syst, Nuclear &nucl) {
  ...
  if (nucl.nnucl != 3.0 * syst.Number_of_atoms) {
    cout << "Error: in system_to_nuclear: dimensions of System and Nuclear "
...
// boost::python::dict map_to_dict(std::map<std::string, double> map_){  return
// <std::string, double>map_to_dict_templ(map_); }
