...
/**
  ...
  \brief The file implements computations of various types of forces and force
  tensors
...
CMATRIX nHamiltonian::forces_adi(vector<int> &act_states) {
  /**
    This only applies for a nHamiltonian object with a set of children whose
    size is equal to the number of integers in `act_states` input
...
CMATRIX nHamiltonian::all_forces_adi(vector<int> &id_) {
  /**
    These are forces on all states for a Hamiltonian of given level, given by
    its id
  */
    ...
    if (id_[0] == id) {
      ...
      for (int n = 0; n < nnucl; n++) {
        if (d1ham_adi_mem_status[n] == 0) {
          cout
              << "Error in all_forces_adi(): the derivatives of the Hamiltonian matrix in the \
                adiabatic basis w.r.t. the nuclear DOF "
              << n << " is not allocated but is \
      ...
      cout << "ERROR in all_forces_adi: No Hamiltonian matching the requested "
              "id\n";
      exit(0);
    }
  } else {
