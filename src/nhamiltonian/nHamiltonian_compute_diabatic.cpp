...
bp::object import_py_funct(const std::string &module, const std::string &path,
                           bp::object &globals) {
  bp::dict locals;
...
void nHamiltonian::compute_diabatic(int model, vector<double> &q,
...
void nHamiltonian::compute_diabatic(int model, vector<double> &q,
                                    vector<double> &params, int lvl) {

  if (level == lvl) {
    /*
        if(model==100){   model_1S_1D_poly2(ham_dia, ovlp_dia, d1ham_dia,
       dc1_dia, q, params);  } if(model==100){   model_1S_1D_poly4(ham_dia,
       ovlp_dia, d1ham_dia, dc1_dia, q, params);  }
    */
  } else if (lvl > level) {
...
// void nHamiltonian::compute_diabatic(bp::object py_funct, bp::object q,
// bp::object params){
void nHamiltonian::compute_diabatic(bp::object py_funct, MATRIX &q,
                                    bp::object params) {
  /**
    Performs the diabatic properties calculation at the top-most level of the
...
void nHamiltonian::compute_diabatic(bp::object py_funct, MATRIX &q,
  ...
  /**
    ...
    "ham_dia" (CMATRIX), "ovld_dia" (CMATRIX), "d1ham_dia" (list of CMATRIX),
    "dc1_dia" (list of CMATRIX) of the specified types
    ...
    Contentwise, this function computes the diabatic properties and populates
