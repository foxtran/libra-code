...
/**
  ...
  \brief The file implements the calculations of the Ehrenfest-related
  properties: energies, forces and state-resolved force contributions
...
complex<double> nHamiltonian::Ehrenfest_energy_dia(CMATRIX &ampl_dia) {
  /**
    ...
    E = <PSI|H|PSI>/<PSI|PSI> = C_dia.H() * <psi_dia|H|psi_dia> * C_dia /
    C_dia.H() * S * C_dia
  ...
  */
  ...
  if (ovlp_dia_mem_status == 0) {
    cout
        << "Error in Ehrenfest_energy_dia(): the overlap matrix in the diabatic basis is not allocated \
  ...
  if (ham_dia_mem_status == 0) {
    cout
        << "Error in Ehrenfest_energy_dia(): the diabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }
...
complex<double> nHamiltonian::Ehrenfest_energy_dia(CMATRIX &ampl_dia,
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return Ehrenfest_energy_dia(ampl_dia);
    } else {
      cout << "ERROR in force_dia: No Hamiltonian matching the requested id\n";
      exit(0);
    }
  } else {
...
complex<double> nHamiltonian::Ehrenfest_energy_adi(CMATRIX &ampl_adi) {
  ...
  */
  ...
  if (ham_adi_mem_status == 0) {
    cout
        << "Error in Ehrenfest_energy_adi(): the adiabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }
...
complex<double> nHamiltonian::Ehrenfest_energy_adi(CMATRIX &ampl_adi,
  ...
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return Ehrenfest_energy_adi(ampl_adi);
    } else {
      cout << "ERROR in force_dia: No Hamiltonian matching the requested id\n";
      exit(0);
    }
...
vector<CMATRIX> nHamiltonian::Ehrenfest_forces_tens_adi(CMATRIX &ampl_adi) {
  /**
    The elements of the returned list are the matrixes F_adi^MF that, when
    multiplied by the adiabatic amplitudes give the Ehrenfest forces in the
    adiabatic basis
  ...
  */
  ...
  vector<CMATRIX> res;
  res = vector<CMATRIX>(nnucl, CMATRIX(nadi, nadi));

  if (ham_adi_mem_status == 0) {
    cout
        << "Error in Ehrenfest_forces_tens_adi(): the adiabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }
...
//  not allocated, but they are needed for the calculations\n"; exit(0); }
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (d1ham_adi_mem_status[n] == 0) {
      cout
          << "Error in Ehrenfest_forces_tens_adi(): the derivatives of the Hamiltonian matrix in the \
    ...
    if (dc1_adi_mem_status[n] == 0) {
      cout
          << "Error in Ehrenfest_forces_tens_adi(): the derivatives couplings matrix in the adiabatic \
...
vector<CMATRIX> nHamiltonian::Ehrenfest_forces_tens_adi(CMATRIX &ampl_adi,
                                                        vector<int> &id_) {
  /**
    See the description of the Ehrenfest_forces_tens_adi(CMATRIX& ampl_adi)
    function
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return Ehrenfest_forces_tens_adi(ampl_adi);
    } else {
      cout << "ERROR in Ehrenfest_forces_adi: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
...
vector<CMATRIX> nHamiltonian::Ehrenfest_forces_tens_dia(CMATRIX &ampl_dia) {
  /**
    The elements of the returned list are the matrixes F_dia^MF that, when
    multiplied by the diabatic amplitudes give the Ehrenfest forces in the
    diabatic basis
  ...
  */
  ...
  vector<CMATRIX> res;
  res = vector<CMATRIX>(nnucl, CMATRIX(ndia, ndia));

  if (ovlp_dia_mem_status == 0) {
    cout
        << "Error in Ehrenfest_forces_tens_dia(): the overlap matrix in the diabatic basis is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }
  ...
  if (ham_dia_mem_status == 0) {
    cout
        << "Error in Ehrenfest_forces_tens_dia(): the diabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
    exit(0);
  }
...
//  not allocated, but they are needed for the calculations\n"; exit(0); }
  ...
  for (int n = 0; n < nnucl; n++) {
    ...
    if (d1ham_dia_mem_status[n] == 0) {
      cout
          << "Error in Ehrenfest_forces_tens_dia(): the derivatives of the Hamiltonian matrix in the \
    ...
    if (dc1_dia_mem_status[n] == 0) {
      cout
          << "Error in Ehrenfest_forces_tens_dia(): the derivatives couplings matrix in the diabatic \
      basis w.r.t. the nuclear DOF "
          << n << " is not allocated but is needed for the calculations \n";
      exit(0);
    }
...
vector<CMATRIX> nHamiltonian::Ehrenfest_forces_tens_dia(CMATRIX &ampl_dia,
                                                        vector<int> &id_) {
  /**
    See the description of the Ehrenfest_forces_tens_dia(CMATRIX& ampl_dia)
    function
  */
  if (id_.size() == 1) {
    if (id_[0] == id) {
      return Ehrenfest_forces_tens_dia(ampl_dia);
    } else {
      cout << "ERROR in Ehrenfest_forces_dia: No Hamiltonian matching the "
              "requested id\n";
      exit(0);
    }
  } else {
