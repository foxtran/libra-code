...
class nHamiltonian {
  /**
    ...
    For each variable X (except for the very trivial cases), we'll have a
    variable "X_mem_status", which which describes the status of the memory for
    that variable:
  ...
  */
  ...
  void check_cmatrix(bp::object obj, std::string matrix_name, int nrows,
                     int ncols);
  void check_cmatrix_list(bp::object obj, std::string matrix_name, int nrows,
...
public:
  ...
  */
  ...
  //  CMATRIX* ampl_dia;          ///< Amplitudes of the diabatic states in the
  //  overal wavefunction int ampl_dia_mem_status;

  CMATRIX *ovlp_dia; ///< the overlap of the diabatic basis
  int ovlp_dia_mem_status;

  vector<CMATRIX *> dc1_dia; ///< first-order derivative coupling matrices in
                             ///< the diabatic basis
  ...
  CMATRIX *nac_dia; ///< Nonadiabatic couplings (time-derivative couplings) in
                    ///< diabatic representation
  ...
  vector<CMATRIX *> d1ham_dia; ///< first order derivatives of the Hamiltonian
                               ///< matrix in the diabatic basis
  vector<int> d1ham_dia_mem_status;

  vector<CMATRIX *> d2ham_dia; ///< second order derivatives of the Hamiltonian
  ...
  /** Adiabatic representation: by definition, the states are orthonormal in
  this representation
  ...
  */
  ...
  vector<CMATRIX *> dc1_adi; ///< first-order derivative coupling matrices in
                             ///< the adiabatic basis
  ...
  CMATRIX *nac_adi; ///< Nonadiabatic couplings (time-derivative couplings) in
  ...
  vector<CMATRIX *> d1ham_adi; ///< first order derivatives of the Hamiltonian
                               ///< matrix in the adiabatic basis (diagonal)
  vector<int> d1ham_adi_mem_status;

  vector<CMATRIX *> d2ham_adi; ///< second order derivatives of the Hamiltonian
                               ///< matrix in the adiabatic basis
  ...
  |psi_dia> * U * den_mat_adi * U.H() * <psi_dia| = |psi_dia> * den_mat_dia *
  <psi_dia|
  ...
  */
  ...
  CMATRIX *basis_transform; ///< These are the eigenvectors of the generalized
                            ///< eigenvalue problem for diabatic Hamiltonian
  int basis_transform_mem_status;

  CMATRIX *time_overlap_adi; ///< These are <psi_i_adi (t) | psi_j_adi (t+dt)>
                             ///< matrices
  int time_overlap_adi_mem_status;

  CMATRIX *time_overlap_dia; ///< These are <psi_i_dia (t) | psi_j_dia (t+dt)>
                             ///< matrices
  int time_overlap_dia_mem_status;

  vector<int> *ordering_adi; ///< the permutation describing how the indices of
                             ///< the internally-stored
                             /// properties like ham_adi or nac_adi relate to
                             /// the indices of the actual "physical" states.

  CMATRIX
      *cum_phase_corr; /// These are the cumulative phase corrections applied to
                       /// basis_transform (this correction is indexed by the
                       /// inernal indexing of states, not physical) Keep in
                       /// mind that these phases have already been applied to
                       /// basis_transform variable (this is the convention)
  int cum_phase_corr_mem_status;

  /**
     All the computation control parameters
  **/
  int eigen_algo; ///< eigensolver algorithm used to convert diabatic to
                  ///< adiabatic basis
                  /// 0 - generalized eigensolver (may reorder states) [default]
                  /// 1 - eigensolver without reordering (assumes diabatic basis
                  /// is orthonormaly)

  double phase_corr_ovlp_tol; /// phase correction overlap tolerance, we only
                              /// compute phase corrections if the time overlaps
                              /// are above (in magnitude) this value, otherwise
                              /// we assume that uncommon state reordering
                              /// (adiabatic state switching) has happened
                              /// (which may occur in the stochastic state
                              /// tracking method) then, we don't apply the
                              /// phase correction since it doesn't make sense
  ...
  **/
  ...
  void
  add_child(nHamiltonian &child); ///< Associate an existing Hamiltonian with
                                  ///< the present one to become its child
  ...
  void copy_level_content(
      nHamiltonian *src); ///< To copy content if all the memory is allocated
  ...
  void compute_diabatic(int model, vector<double> &q, vector<double> &params,
                        int lvl); // for internal model types
  void compute_diabatic(int model, vector<double> &q,
                        vector<double> &params); // for internal model types

  //  void compute_diabatic(bp::object py_funct, bp::object q, bp::object
  //  params, int lvl); // for models defined in Python void
  //  compute_diabatic(bp::object py_funct, bp::object q, bp::object params); //
  //  for models defined in Python
  void compute_diabatic(bp::object py_funct, MATRIX &q, bp::object params,
                        int lvl); // for models defined in Python
  void compute_diabatic(bp::object py_funct, MATRIX &q,
                        bp::object params); // for models defined in Python
  ...
  void add_ethd3_dia(const MATRIX &q, const MATRIX &invM, double alp,
                     int der_lvl);
  void add_ethd3_adi(const MATRIX &q, const MATRIX &invM, double alp,
                     int der_lvl);
  void add_ethd3_adi(const MATRIX &q, const MATRIX &p, const MATRIX &invM,
                     double alp, double bet, int der_lvl);
  ...
  */
  ...
  //  void compute_adiabatic(bp::object py_funct, bp::object q, bp::object
  //  params, int lvl); // for models defined in Python void
  //  compute_adiabatic(bp::object py_funct, bp::object q, bp::object params);
  //  // for models defined in Python
  void compute_adiabatic(bp::object py_funct, MATRIX &q, bp::object params,
                         int lvl); // for models defined in Python
  void compute_adiabatic(bp::object py_funct, MATRIX &q,
                         bp::object params); // for models defined in Python
  ...
  CMATRIX forces_adi(vector<int> &act_states); // -dH_adi/dR in the adiabatic
                                               // basis for several trajectories
  CMATRIX forces_dia(vector<int> &act_states); // -dH_dia/dR in the diabatic
                                               // basis for several trajectories

  CMATRIX all_forces_adi(vector<int> &id_);

  /*
    These variables are poorly-defined and unnecessary, so we are going to
    deprecate them, AVA 9/15/2022 Instead - there will be functions to extract
    adiabatic and diabatic forces
    ...
    CMATRIX forces_adi(CMATRIX& ampl_adi);  // -dE/dR in the adiabatic basis,
    assuming Cadi = Cadi(t) CMATRIX forces_adi(CMATRIX& ampl_adi, vector<int>&
    id_);  // -dE/dR in the adiabatic basis, assuming Cadi = Cadi(t) CMATRIX
    forces_adi(vector<int>& act_states);   // -dE/dR in the adiabatic basis for
    several trajectories CMATRIX forces_adi(int act_state);   // -dE/dR in the
    adiabatic basis for several trajectories, all in the same state

    CMATRIX forces_dia(CMATRIX& ampl_dia);  // -dE/dR in the diabatic basis,
    assuming Cdia = Cdia(t) CMATRIX forces_dia(CMATRIX& ampl_dia, vector<int>&
    id_);  // -dE/dR in the diabatic basis, assuming Cdia = Cdia(t) CMATRIX
    forces_dia(vector<int>& act_states);   // -dE/dR in the diabatic basis for
    several trajectories CMATRIX forces_dia(int act_state);   // -dE/dR in the
    diabatic basis for several trajectories, all in the same state
  */
  ...
  ///< In nHamiltonian_compute_Ehrenfest.cpp and
  ///< nHamiltonian_compute_Ehrenfest_forces.cpp
  complex<double> Ehrenfest_energy_dia(CMATRIX &ampl_dia);
  complex<double> Ehrenfest_energy_dia(CMATRIX &ampl_dia, vector<int> &id_);
  CMATRIX
  Ehrenfest_forces_dia_unit(CMATRIX &ampl_dia,
                            int option); ///< Ehrenfest forces in diabatic basis
  CMATRIX Ehrenfest_forces_dia_unit(
      CMATRIX &ampl_dia); ///< Ehrenfest forces in diabatic basis
  CMATRIX
  Ehrenfest_forces_dia(CMATRIX &ampl_dia, int lvl,
                       int option); ///< Ehrenfest forces in diabatic basis
  CMATRIX Ehrenfest_forces_dia(CMATRIX &ampl_dia,
                               int lvl); ///< Ehrenfest forces in diabatic basis
  ...
  CMATRIX Ehrenfest_forces_adi_unit(
      CMATRIX &ampl_adi, int option); ///< Ehrenfest forces in adiabatic basis
  CMATRIX Ehrenfest_forces_adi_unit(
      CMATRIX &ampl_adi); ///< Ehrenfest forces in adiabatic basis
  CMATRIX
  Ehrenfest_forces_adi(CMATRIX &ampl_adi, int lvl,
                       int option); ///< Ehrenfest forces in adiabatic basis
  CMATRIX
  Ehrenfest_forces_adi(CMATRIX &ampl_adi,
                       int lvl); ///< Ehrenfest forces in adiabatic basis

  vector<CMATRIX> Ehrenfest_forces_tens_adi(
      CMATRIX &
          ampl_adi); // Force tensor in adiabatic basis, assuming Cadi = Cadi(t)
  vector<CMATRIX> Ehrenfest_forces_tens_adi(
      CMATRIX &ampl_adi,
      vector<int>
          &id_); // Force tensor in adiabatic basis, assuming Cadi = Cadi(t)
  vector<CMATRIX> Ehrenfest_forces_tens_dia(
      CMATRIX
          &ampl_dia); // Force tensor in diabatic basis, assuming Cdia = Cdia(t)
  vector<CMATRIX> Ehrenfest_forces_tens_dia(
      CMATRIX &ampl_dia,
      vector<int>
          &id_); // Force tensor in diabatic basis, assuming Cdia = Cdia(t)
...
typedef std::vector<nHamiltonian>
    nHamiltonianList; ///< data type for keeping a list of generic Hamiltonians
...
double ETHD3_energy(const MATRIX &q, const MATRIX &p, const MATRIX &invM,
                    double alp, double bet);

MATRIX ETHD3_forces(const MATRIX &q, const MATRIX &invM,
                    double alp); // -dH_{ETHD3}/dQ
MATRIX ETHD3_forces(const MATRIX &q, const MATRIX &p, const MATRIX &invM,
                    double alp, double bet); // -dH_{ETHD3}/dQ

MATRIX ETHD3_friction(const MATRIX &q, const MATRIX &p, const MATRIX &invM,
                      double alp, double bet); // dH_{ETHD3}/dP
