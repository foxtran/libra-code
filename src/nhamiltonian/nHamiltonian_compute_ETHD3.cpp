...
/**
  ...
  Trajectories Hamiltonian Dynamics) terms. This new, experimental approach.
...
double ETHD3_energy(const MATRIX &q, const MATRIX &invM, double alp) {
  ...
  */
  ...
  for (traj_k = 0; traj_k < ntraj; traj_k++) {
    ...
    MATRIX d1rho_k(ndof, 1); // derivatives of the total density at the position
                             // of trajectory k (Q_k) w.r.t. q_{dof}
    MATRIX d2rho_k(ndof, 1); // second derivatives of the total density at the
                             // position of trajectory k (Q_k) w.r.t. q_{dof}
    ...
    for (traj_j = 0; traj_j < ntraj; traj_j++) {
      ...
      for (dof = 0; dof < ndof; dof++) {
        ...
        d2rho_k.add(dof, 0,
                    (4.0 * alp * alp * dq.get(dof) * dq.get(dof) - 2.0 * alp) *
                        g_kj);
      } // for dof
  ...
  */
  ...
  for (traj_k = 0; traj_k < ntraj; traj_k++) {
    ...
    for (traj_j = 0; traj_j < ntraj; traj_j++) {
      ...
      double g_kj =
          exp(-alp * (dq.T() * dq).get(0)) * exp(-bet * (dp.T() * dp).get(0));
  ...
  for (traj_k = 0; traj_k < ntraj; traj_k++) {
    for (traj_j = 0; traj_j < ntraj; traj_j++) {
      ...
      for (dof_i = 0; dof_i < ndof; dof_i++) {
        ...
        d2rho.add(
            dof_i, traj_k,
            (4.0 * alp * alp * dq.get(dof_i) * dq.get(dof_i) - 2.0 * alp) *
                g_kj);
      } // for dof
  ...
  for (traj_k = 0; traj_k < ntraj; traj_k++) {
    for (traj_j = 0; traj_j < ntraj; traj_j++) {
      ...
      for (dof_i = 0; dof_i < ndof; dof_i++) {
        ...
        A.add(0, traj_k,
              d1rho.get(dof_i, traj_j) * invM.get(dof_i, 0) *
                  d1rho.get(dof_i, traj_j));
      ...
      for (traj_k = 0; traj_k < ntraj; traj_k++) {
        ...
        for (traj_j = 0; traj_j < ntraj; traj_j++) {
          if (traj_n == traj_j || traj_n == traj_k) {
            ...
            double pref = (q.get(dof_a, traj_k) - q.get(dof_a, traj_j)) *
                          g.get(traj_k, traj_j);
        ...
        for (dof_i = 0; dof_i < ndof; dof_i++) {
          ...
          for (traj_j = 0; traj_j < ntraj; traj_j++) {
            if (traj_n == traj_j || traj_n == traj_k) {
              ...
              double pref = -2.0 * alp *
                            (q.get(dof_i, traj_k) - q.get(dof_i, traj_j)) *
                            (q.get(dof_a, traj_k) - q.get(dof_a, traj_j));
              if (dof_i == dof_a) {
                pref += 1.0;
              }
          ...
          dAk_dQan += 2.0 * d1rho.get(dof_i, traj_k) * invM.get(dof_i, 0) *
                      drho_prime_ik_dQan;
        ...
        double val_k =
            ((dAk_dQan + 2.0 * Bk * drhok_dQan) * rhok2 -
             2.0 * Ak * rhok * drhok_dQan - 2.0 * dBk_dQan * rhok2 * rhok);
        val_k /= rhok4;
      ...
      for (traj_k = 0; traj_k < ntraj; traj_k++) {
        ...
        for (traj_j = 0; traj_j < ntraj; traj_j++) {
          if (traj_n == traj_j || traj_n == traj_k) {
            ...
            double pref = (p.get(dof_a, traj_k) - p.get(dof_a, traj_j)) *
                          g.get(traj_k, traj_j);
        ...
        for (dof_i = 0; dof_i < ndof; dof_i++) {
          ...
          for (traj_j = 0; traj_j < ntraj; traj_j++) {
            ...
            if (traj_n == traj_j || traj_n == traj_k) {
              ...
              double pref = (q.get(dof_i, traj_k) - q.get(dof_i, traj_j)) *
                            (p.get(dof_a, traj_k) - p.get(dof_a, traj_j));

              pref *= g.get(traj_k, traj_j);

              if (traj_n == traj_k) {
                drho_prime_ik_dPan += pref;
              }
              if (traj_n == traj_j) {
                drho_prime_ik_dPan -= pref;
              }
          ...
          dAk_dPan += 2.0 * d1rho.get(dof_i, traj_k) * invM.get(dof_i, 0) *
                      drho_prime_ik_dPan;
        ...
        double val_k =
            ((dAk_dPan + 2.0 * Bk * drhok_dPan) * rhok2 -
             2.0 * Ak * rhok * drhok_dPan - 2.0 * dBk_dPan * rhok2 * rhok);
        val_k /= rhok4;
...
void nHamiltonian::add_ethd3_dia(const MATRIX &q, const MATRIX &invM,
                                 double alp, int der_lvl) {
  /**
    Add ETHD3 energies and (optionally) forces to all the children Hamiltonians
...
void nHamiltonian::add_ethd3_adi(const MATRIX &q, const MATRIX &invM,
                                 double alp, int der_lvl) {
  /**
    Add ETHD3 energies and (optionally) forces to all the children Hamiltonians
...
void nHamiltonian::add_ethd3_adi(const MATRIX &q, const MATRIX &p,
