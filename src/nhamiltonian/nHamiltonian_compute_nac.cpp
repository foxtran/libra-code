...
void nHamiltonian::compute_nac_dia(MATRIX &p, const MATRIX &invM, int lvl,
  ...
  /**
    ...
    perform the transformation \param[in] split The flag tells the function to
    perform the calculations column by column, with column being used by a
    children Hamiltonian (next level relative to the input parameter lvl). This
    is only allowed if the number of columns in p is equal to the number of
    children of the present-level Hamiltonian.
