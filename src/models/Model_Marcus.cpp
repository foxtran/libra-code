...
void Marcus_Ham(double x, MATRIX *H, MATRIX *dH, MATRIX *d2H,
                vector<double> &params) {
  // Marcus spin-boson Hmiltonian in diabatic representation

  if (H->n_elts != 4) {
    std::cout << "Error in Marcus_Ham: H matrix must be allocated\n";
    exit(0);
  }
  if (dH->n_elts != 4) {
    std::cout << "Error in Marcus_Ham: dH matrix must be allocated\n";
    exit(0);
  }
  if (d2H->n_elts != 4) {
    std::cout << "Error in Marcus_Ham: d2H matrix must be allocated\n";
    exit(0);
  }
