...
void Rabi2_Ham(double x, MATRIX *H, MATRIX *dH, MATRIX *d2H,
               vector<double> &params) {
  // Rabi2 hamiltonian in diabatic representation

  if (H->n_elts != 4) {
    std::cout << "Error in Rabi2_Ham: H matrix must be allocated\n";
    exit(0);
  }
  if (dH->n_elts != 4) {
    std::cout << "Error in Rabi2_Ham: dH matrix must be allocated\n";
    exit(0);
  }
  if (d2H->n_elts != 4) {
    std::cout << "Error in Rabi2_Ham: d2H matrix must be allocated\n";
    exit(0);
  }
