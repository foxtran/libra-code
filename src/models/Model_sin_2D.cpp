...
void sin_2D_Ham(double x, double y, MATRIX *H, MATRIX *dH1, MATRIX *dH2,
  ...
  if (H->n_elts != 4) {
    std::cout << "Error in sin_2D_Ham: H matrix must be allocated\n";
    exit(0);
  }
  if (dH1->n_elts != 4) {
    std::cout << "Error in sin_2D_Ham: dH1 matrix must be allocated\n";
    exit(0);
  }
  if (d2H1->n_elts != 4) {
    std::cout << "Error in sin_2D_Ham: d2H1 matrix must be allocated\n";
    exit(0);
  }
  if (dH2->n_elts != 4) {
    std::cout << "Error in sin_2D_Ham: dH2 matrix must be allocated\n";
    exit(0);
  }
  if (d2H2->n_elts != 4) {
    std::cout << "Error in sin_2D_Ham: d2H2 matrix must be allocated\n";
    exit(0);
  }
...
boost::python::list sin_2D_Ham(double x, double y,
                               boost::python::list params_) {
  // sin_2D hamiltonian in diabatic representation
