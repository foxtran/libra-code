...
public:
  ...
  //------------------- Initializations ---------------------------------
  void init(double t, double x, double y, double z) {
    Lt = t;
    Lx = x;
    Ly = y;
    Lz = z;
  ...
  QUATERNION inverse() {
    ...
    if (norm > 0.0) {
      ...
      cout << "Zero quaternion norm! Can not divide!" << endl;
    }
    return tmp;
  }
  QUATERNION conj() {
  ...
  void normalize() {
    double norm = sqrt(Lt * Lt + Lx * Lx + Ly * Ly + Lz * Lz);
    Lt = Lt / norm;
    Lx = Lx / norm;
    Ly = Ly / norm;
    Lz = Lz / norm;
  ...
  */
  ...
  operator*(const QUATERNION &q,
            const VECTOR &v); // Multiplication of vector and quaternion;
  friend QUATERNION operator*(
      const MATRIX &m,
      const QUATERNION &q); // Multiplication of Matrix(4x4) and quaternion
                            // defined similar to multiplication of 4D vector
  //  friend QUATERNION operator*(const MATRIX& m, const QUATERNION& q); //
  ...
  operator*(const VECTOR &v,
            const QUATERNION &q); // Multiplication of vector and quaternion;
  friend double dot_prod(
      const QUATERNION &,
      const QUATERNION &); // Normal dot product of 2 quaternions as 4D vectors
  //  friend double dot_prod(const QUATERNION&, const QUATERNION&);

  friend bool operator==(const QUATERNION &q1, const QUATERNION &q2) {
    // Are matrices equal
    return ((q1.Lt == q2.Lt) && (q1.Lx == q2.Lx) && (q1.Ly == q2.Ly) &&
            (q1.Lz == q2.Lz));
  }
...
//-------- IO functions --------
void set_value(int &defined, QUATERNION &value, boost::python::object obj,
               std::string attrName);
