...
void solve_linsys(CMATRIX &C, CMATRIX &D, CMATRIX &X, double eps, int maxiter,
                  double omega) {
  /**
  ...
  */
  ...
  if (C.n_rows != D.n_rows) {
    std::cout << "Error: The number of rows of matrices C and D in equation CX "
                 "= D must be equal\n";
    exit(35);
  } // n
  if (C.n_cols != X.n_rows) {
    std::cout << "Error: The number of cols of CMATRIX C and num of rows in "
                 "CMATRIX D in equation CX = D must be equal\n";
    exit(35);
  } // m
  if (X.n_cols != D.n_cols) {
    std::cout << "Error: The number of cols of matrices X and D in equation CX "
                 "= D must be equal\n";
    exit(35);
  } // p
  ...
  while ((error > eps) && (iter < maxiter)) {
    ...
    for (k = 0; k < p; k++) {
      ...
      for (i = 0; i < m; i++) {
        int indx = i * p + k;
        error +=
            ((std::conj(x.M[i] - xprev.M[i])) * (x.M[i] - xprev.M[i])).real();
...
void solve_linsys1(CMATRIX &C, CMATRIX &X, double eps, int maxiter,
                   double omega) {
  /**
  ...
  while ((error > eps) && (iter < maxiter)) {
    ...
    for (k = 0; k < p; k++) {
      ...
      for (i = 0; i < m; i++) {
        ...
        X.M[i] = omega * x.M[i] +
                 (1.0 - omega) *
                     X.M[i]; // k takes value of only 0, p = 1, so i*p+k = i
      }                      // for i
  ...
  if (N > 2) {
    ...
    for (int k = 0; k < Nhalf; k++) {
      ...
      p1 *= p2;
           ...
           2) { // No more recursive levels below this one - do all explicitly
    cft1(in, out, xmin, kmin, dx);
  }
}

void cft1_2D(CMATRIX &in, CMATRIX &out, double xmin, double ymin, double kxmin,
             double kymin, double dx, double dy) {
  /**
    ...
    in and out - are now 2D matrices (columns and rows) - this is for
    convenience, although they both may be reformulated as 1D matrices (colums
    or rows)
    ...
    \param[in] kxmin The minimal boundary of the reciprocal-space grid along X
    direction \param[in] kymin The minimal boundary of the reciprocal-space grid
    along Y direction \param[in] dx Spacing between points in the real space,
    along the X direction \param[in] dy Spacing between points in the real
    space, along the Y direction
  ...
  */
  ...
  // Conceptually:  out.M[kx*Ny+ky] +=  in.M[nx*Ny+ny] *
  // exp(-2.0*M_PI*one*(Kx*Rx + Ky*Ry));
...
void cfft1_2D(CMATRIX &in, CMATRIX &out, double xmin, double ymin, double kxmin,
              double kymin, double dx, double dy) {
  /**
  ...
  */
  ...
  // Case 2
    ...
    for (int kx = 0; kx < Nxhalf; kx++) {
      for (int ky = 0; ky < Ny; ky++) {
        ...
        out.M[(kx + Nxhalf) * Ny + ky] =
            0.5 * eax * (out_e.M[indx] - p1x * out_o.M[indx]);
  ...
  // Case 3
    ...
    for (int ky = 0; ky < Nyhalf; ky++) {
      for (int kx = 0; kx < Nx; kx++) {
        ...
        out.M[kx * Ny + (ky + Nyhalf)] =
            0.5 * eay * (out_e.M[indx] - p1y * out_o.M[indx]);
  ...
  else if (Nx == 2 && Ny == 2) { // No more recursive levels below this one - do
                                 // all explicitly
...
void cft2(CMATRIX &in, CMATRIX &out, double xmin, double kmin, double dx,
          double dk) {
  /**
    Continuous Fourier Transform
    f(k) = Integral ( f(r) * exp(-2*pi*i*k*r) * dr ) =
    ...
    \param[in] xmin The minimal boundary of the real-space grid along X axis
    \param[in] kxmin The minimal boundary of the reciprocal-space grid along X
    direction \param[in] dx Spacing between points in the real space, along the
    X direction \param[in] dk Spacing between points in the reciprocal space,
    along the X direction


    "in" is 1 x Nr or Nr x 1 matrix , Nr - the number of grid points in the real
    space "out" is 1 x Nk or Nk x 1 matrix, Nk - the number of grid points in
...
void inv_cft(CMATRIX &in, CMATRIX &out, double xmin, double dx) {
  /**
    ...
    \param[out] out The output matrix
    \param[in] xmin The minimal boundary of the real-space grid along X axis
    \param[in] dx Spacing between points in the real space, along the X
    direction
...
void inv_cft1_2D(CMATRIX &in, CMATRIX &out, double xmin, double ymin,
  ...
  */
  ...
  // Conceptually:  out.M[nx*Ny+ny] +=  in.M[kx*Ny+ky] * exp(2.0*M_PI*one*(Kx*Rx
  // + Ky*Ry));
...
void inv_cfft1_2D(CMATRIX &in, CMATRIX &out, double xmin, double ymin,
                  double kxmin, double kymin, double dx, double dy) {
  /**
    ...
    for (int nx = 0; nx < Nxhalf; nx++) {
      ...
      for (int ny = 0; ny < Nyhalf; ny++) {
        ...
        out.M[nx * Ny + ny] =
            0.25 * (out_ee.M[indx] + p1x * out_oe.M[indx] +
                    p1y * out_eo.M[indx] + p1x * p1y * out_oo.M[indx]);
        out.M[(nx + Nxhalf) * Ny + ny] =
            0.25 * eax *
            (out_ee.M[indx] - p1x * out_oe.M[indx] + p1y * out_eo.M[indx] -
             p1x * p1y * out_oo.M[indx]);
        out.M[nx * Ny + (ny + Nyhalf)] =
            0.25 * eay *
            (out_ee.M[indx] + p1x * out_oe.M[indx] - p1y * out_eo.M[indx] -
             p1x * p1y * out_oo.M[indx]);
...
void inv_cft2(CMATRIX &in, CMATRIX &out, double xmin, double kmin, double dx,
              double dk) {
  /**
    Inverse Continuous Fourier Transform
    ...
    = dk * exp(2*pi*i*( kmin * r_n)) * sum ( in[k] * exp(2*pi*i*dk*[k]*r_n)  ) =
    f[n] [k]
...
void convolve(CMATRIX &f, CMATRIX &g, CMATRIX &conv, double dx) {
  /**
    Convolve two Fourier transforms
     conv(k) = integral(  f(k') * g(k-k') ) dk' = sum (n'/L) * f[n'] * g[n-n'] =
    conv[n] n' n, n' - integers,  k = n/L, k' = n'/L L = dx * N, N - size of the
    set <in> and <out> are the vectors with n elements: n x 1
  */
