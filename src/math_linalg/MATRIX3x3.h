...
public:
  ...
  void diag(double x, double y, double z) {
    xy = xz = yx = yz = zx = zy = 0.0;
    xx = x;
    yy = y;
    zz = z;
  ...
  void Rotation(const VECTOR &r); // Create a rotation matrix for
                                  // rotation around direction given by
                                  // direction of the argument vector, and on
                                  // the amount given by the norm of the vector
  ...
  double Determinant() {
    return (xx * (yy * zz - yz * zy) - yx * (xy * zz - zy * xz) +
            zx * (xy * yz - yy * xz));
  }
  ...
  MATRIX3x3 operator*(MATRIX3x3 ob) const {
    MATRIX3x3 m;
    m.xx = xx * ob.xx + xy * ob.yx + xz * ob.zx;
    m.xy = xx * ob.xy + xy * ob.yy + xz * ob.zy;
    m.xz = xx * ob.xz + xy * ob.yz + xz * ob.zz;
    m.yx = yx * ob.xx + yy * ob.yx + yz * ob.zx;
    m.yy = yx * ob.xy + yy * ob.yy + yz * ob.zy;
    m.yz = yx * ob.xz + yy * ob.yz + yz * ob.zz;
    m.zx = zx * ob.xx + zy * ob.yx + zz * ob.zx;
    m.zy = zx * ob.xy + zy * ob.yy + zz * ob.zy;
    m.zz = zx * ob.xz + zy * ob.yz + zz * ob.zz;
    return m;
  }
  MATRIX3x3 operator+(MATRIX3x3 ob) const {
    MATRIX3x3 m;
  ...
  friend istream &operator>>(istream &strm, MATRIX3x3 &ob) { return strm; }
  ...
  operator*(const MATRIX3x3 &m,
            const VECTOR &v); // Multiplication of vector and matrix;
};
...
//-------- IO functions --------
void set_value(int &defined, MATRIX3x3 &value, boost::python::object obj,
               std::string attrName);
