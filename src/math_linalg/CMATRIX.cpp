...
/**
  \file CMATRIX.cpp
  \brief The file implements the CMATRIX class for representing arbitrary size
...
CMATRIX::CMATRIX(vector<vector<double>> &re_part,
                 vector<vector<double>> &im_part) {
  /**
    Constructor: creates a CMATRIX from 2 2D-arrays - real and imaginary parts
  */

  if (re_part.size() != im_part.size()) {
    cout << "Error in CMATRIX constructor: y-dimensions(num of rows) of the "
            "real and imaginary arrays are not equal\n";
    exit(0);
  }
  if (re_part[0].size() != im_part[0].size()) {
    cout << "Error in CMATRIX constructor: x-dimensions(num of cols) of the "
            "real and imaginary arrays are not equal\n";
    exit(0);
  }
  n_rows = re_part.size();
...
CMATRIX::CMATRIX(MATRIX &re_part) {
  ...
  for (int i = 0; i < n_rows; i++) {
    for (int j = 0; j < n_cols; j++) {
      M[n] = complex<double>(re_part.get(i, j), 0.0);
      n++;
    }
  }
}
...
CMATRIX::CMATRIX(MATRIX &re_part, MATRIX &im_part) {
  ...
  if (re_part.n_rows != im_part.n_rows) {
    cout << "Error in CMATRIX constructor: num_of_rows of the real and "
            "imaginary matrices are not equal\n";
    exit(0);
  }
  if (re_part.n_cols != im_part.n_cols) {
    cout << "Error in CMATRIX constructor: num_of_cols of the real and "
            "imaginary arrays are not equal\n";
    exit(0);
  }
  n_rows = re_part.n_rows;
  n_cols = re_part.n_cols;
  ...
  for (int i = 0; i < n_rows; i++) {
    for (int j = 0; j < n_cols; j++) {
      M[n] = complex<double>(re_part.get(i, j), im_part.get(i, j));
      n++;
    }
  }
}
...
void CMATRIX::get_components(MATRIX &re_part, MATRIX &im_part) {
  if (re_part.n_cols != n_cols) {
    std::cout << "Error in CMATRIX::get_components : The number of columns of "
                 "the target real component matrix ("
              << re_part.n_cols
              << ") is not equal to the number of columns of the source "
                 "complex matrix ("
              << n_cols << ")\n";
    exit(0);
  }
  if (im_part.n_cols != n_cols) {
    std::cout << "Error in CMATRIX::get_components : The number of columns of "
                 "the target imaginary component matrix ("
              << im_part.n_cols
              << ") is not equal to the number of columns of the source "
                 "complex matrix ("
              << n_cols << ")\n";
  ...
  if (re_part.n_rows != n_rows) {
    std::cout
        << "Error in CMATRIX::get_components : The number of rows of the "
           "target real component matrix ("
        << re_part.n_rows
        << ") is not equal to the number of rows of the source complex matrix ("
        << n_rows << ")\n";
    exit(0);
  }
  if (im_part.n_rows != n_rows) {
    std::cout
        << "Error in CMATRIX::get_components : The number of rows of the "
           "target imaginary component matrix ("
        << im_part.n_rows
        << ") is not equal to the number of rows of the source complex matrix ("
        << n_rows << ")\n";
    exit(0);
  ...
  for (int i = 0; i < n_elts; i++) {
    re_part.M[i] = M[i].real();
    im_part.M[i] = M[i].imag();
  }
}
  ...
  for (int i = 0; i < n_cols - 1; i++) {
    for (int j = i + 1; j < n_cols; j++) {
      ...
      for (int k = 0; k < n_rows; k++) {
        aa += std::conj(M[k * n_cols + i]) * M[k * n_cols + j];
      }
...
void CMATRIX::FindMaxNondiagonalElement(int &row, int &col,
                                        complex<double> &value) {
  /** Finds the maximal (in absolute value) off-diagonal element and its
   * position  */
