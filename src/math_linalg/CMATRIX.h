...
/**
  \file CMATRIX.h
  \brief The file describes the CMATRIX class for representing arbitrary size
  complex-valued matrices as well as the set of functions of complex Fourier
  transforms and convolution

  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  NOTE:  the functions that return CMATRIX should not be relpaced by the base
  template functions

  for some reason, the base_matrix< complex<double> > is not recognized as
  CMATRIX so
...
public:
  ...
  */
  ...
  ///< Create the complex-valued matrix from two tables: one for real, one for
  ///< imaginary components
  CMATRIX(vector<vector<double>> &re_part, vector<vector<double>> &im_part);

  ///< Create the complex-valued matrix (with zero imaginary components) from an
  ///< int-valued matrix
  //  CMATRIX(IMATRIX& re_part);
  //  CMATRIX(const IMATRIX& re_part);

  ///< Create the complex-valued matrix (with zero imaginary components) from a
  ///< real-valued matrix
  CMATRIX(MATRIX &re_part);
  CMATRIX(const MATRIX &re_part);

  ///< Create the complex-valued matrix from two real-valued matrices: one for
  ///< real, one for imaginary components
  ...
  ///< Sets the indx's emelent of the M array to the input value (real and
  ///< imaginary components)
  ...
  CMATRIX
  T(); ///< Returns the matrix which is transposed w.r.t. the caller matrix
  CMATRIX H(); ///< Returns the matrix which is Hermitian conjugate w.r.t. the
               ///< caller matrix
  CMATRIX conj(); ///< Returns the matrix which is complex conjugate w.r.t. the
                  ///< caller matrix
  ...
  void get_components(
      MATRIX &re_part,
      MATRIX &im_part); ///< Split the matrix into real and imaginary components
  ...
  void max_col_elt(int, complex<double> &,
                   int &); ///< Finds the maximal element (in abs. value) and
                           ///< its index in a given column
  void min_col_elt(int, complex<double> &,
                   int &); ///< Finds the maximal element (in abs. value) and
                           ///< its index in a given column
  void max_row_elt(int, complex<double> &,
                   int &); ///< Finds the maximal element (in abs. value) and
                           ///< its index in a given row
  void min_row_elt(int, complex<double> &,
                   int &); ///< Finds the maximal element (in abs. value) and
                           ///< its index in a given row
  boost::python::list
  max_col_elt(int); ///< Finds the maximal element (in abs. value) and its index
                    ///< in a given column
  boost::python::list
  min_col_elt(int); ///< Finds the maximal element (in abs. value) and its index
                    ///< in a given column
  boost::python::list max_row_elt(int); ///< Finds the maximal element (in abs.
                                        ///< value) and its index in a given row
  boost::python::list min_row_elt(int); ///< Finds the maximal element (in abs.
                                        ///< value) and its index in a given row
...
typedef std::vector<CMATRIX>
    CMATRIXList; ///< Data type holding a list of arbitrary-size complex-valued
                 ///< matrices
typedef std::vector<vector<CMATRIX>>
    CMATRIXMap; ///< Data type for storing the table (grid) of the
                ///< arbitrary-size complex-valued matrices
