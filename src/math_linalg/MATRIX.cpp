...
/**
  \file MATRIX.cpp
  \brief The file implements the MATRIX class for representing arbitrary
...
void set_value(int &is_defined, MATRIX &value, boost::python::object obj,
               std::string attrName) {
...
void save(boost::property_tree::ptree &pt, std::string path,
          char path_separator, vector<MATRIX> &vt) {
  int sz = vt.size();
  for (int i = 0; i < sz; i++) {
    stringstream ss(stringstream::in | stringstream::out);
    std::string rt;
...
// ----------- Load --------------
void load(boost::property_tree::ptree &pt, std::string path, MATRIX &vt,
          int &status) {
...
MATRIX load(boost::property_tree::ptree &pt, std::string path, int &status) {
  ...
  libio::load(pt, path + ".n_rows", n_rows, st);
  if (st == 1) {
    status = 1;
  }
  libio::load(pt, path + ".n_cols", n_cols, st);
  if (st == 1) {
    status = 1;
  }
  libio::load(pt, path + ".n_elts", n_elts, st);
  if (st == 1) {
    status = 1;
  }
  ...
  for (int i = 0; i < vt.n_elts; i++) {
    ...
    libio::load(pt, path + "." + rt, vt.M[i], st);
    if (st == 1) {
      status = 1;
    }
  }
...
void load(boost::property_tree::ptree &pt, std::string path,
          char path_separator, MATRIX &vt, int &status) {
  std::cout << "Sorry: load function for MATRIX object is not defined yet\n";
  exit(0);
}

void load(boost::property_tree::ptree &pt, std::string path, vector<MATRIX> &vt,
          int &status) {
  ...
  try {
    BOOST_FOREACH (boost::property_tree::ptree::value_type &v,
                   pt.get_child(path)) {
      MATRIX x(load(pt, path + "." + v.first, st));
      if (st == 1) {
        vt.push_back(x);
        status = 1;
      }
...
void load(boost::property_tree::ptree &pt, std::string path,
          char path_separator, vector<MATRIX> &vt, int &status) {
  std::cout
      << "Sorry: load function for vector<MATRIX> object is not defined yet\n";
  exit(0);
}
