...
/**
  \file IMATRIX.h
  \brief The file describes the IMATRIX class for representing arbitrary size
  integer-valued matrices

  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  NOTE:  the functions that return IMATRIX should not be relpaced by the base
  template functions
...
public:
  ...
  */
  ...
  ///< Create the int-valued matrix from a real-valued matrix by converting the
  ///< numbers to ints
  //  IMATRIX(MATRIX& mtx);
  ...
  /*
    ...
    (real and imaginary components) void set(int row, int col, int value){
      set(row, col, value);
    }
  ...
  */
  ...
  void max_col_elt(int, int &,
                   int &); ///< Finds the maximal element (in abs. value) and
                           ///< its index in a given column
  void min_col_elt(int, int &,
                   int &); ///< Finds the maximal element (in abs. value) and
                           ///< its index in a given column
  void max_row_elt(int, int &, int &); ///< Finds the maximal element (in abs.
                                       ///< value) and its index in a given row
  void min_row_elt(int, int &, int &); ///< Finds the maximal element (in abs.
                                       ///< value) and its index in a given row
  boost::python::list
...
typedef std::vector<IMATRIX>
    IMATRIXList; ///< Data type holding a list of arbitrary-size complex-valued
                 ///< matrices
typedef std::vector<vector<IMATRIX>>
    IMATRIXMap; ///< Data type for storing the table (grid) of the
