...
void export_libint2_wrappers_objects() {
  ...
  */
  /*
    ...
    &electron_repulsion_integral; boost::python::list
    (*expt_electron_repulsion_integral_AO_v3) ( AO& AOa, AO& AOb, AO& AOc, AO&
    AOd, int is_normalize, int is_derivs) = &electron_repulsion_integral;
    ...
    void (PrimitiveG::*expt_set_position_PrimitiveG_v1)(VECTOR) =
    &PrimitiveG::set_position; void (AO::*expt_set_position_AO_v1)(VECTOR) =
    &AO::set_position;
  ...
  */
  ...
  void (*expt_add_to_shell_v1)(
      std::vector<libint2::Shell> & shells, int l_val, bool is_spherical,
      const std::vector<double> &exponents, const std::vector<double> &coeff,
      VECTOR &coords) = &add_to_shell;

  def("add_to_shell", expt_add_to_shell_v1);

  void (*expt_print_shells_v1)(std::vector<libint2::Shell> & shells) =
      &print_shells;
  ...
  (const std::vector<libint2::Shell> &shells_1,
   const std::vector<libint2::Shell> &shells_2, int number_of_threads) =
      &compute_overlaps;
  ...
  /*
    ...
    (const std::vector<libint2::Shell>& shells_1, const
    std::vector<libint2::Shell>& shells_2) = &compute_overlaps_serial;
  ...
  */
  /*
     ...
     py::return_value_policy::reference_internal);
      m.def("compute_1body_ints_parallel", &compute_1body_ints_parallel, "A
     function for computing AO overlaps parallel",
     py::return_value_policy::reference_internal); m.def("compute_1body_ints",
     &compute_1body_ints, "A function for computing integrals serial",
     py::return_value_policy::reference_internal); m.def("initialize_shell",
     &initialize_shell, "A function for creating shells",
     py::return_value_policy::reference_internal); m.def("initialize_atom",
     &initialize_atom, "A function for initializing Atom",
     py::return_value_policy::reference_internal); m.def("add_to_shell",
     &add_to_shell, "A function to add the exponents, contraction coefficients,
     and Atom structure to a predefined shell",
     py::return_value_policy::reference_internal); m.def("print_shell",
     &print_shell, "A function for printing a shell",
     py::return_value_policy::reference_internal); m.def("compute_overlaps",
     &compute_overlaps, "A function that computes overlaps between pairs of
     shells", py::return_value_policy::reference_internal);
