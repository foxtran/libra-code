...
/**
  ...
  \brief The file implements RigidBody class interfaces - initializers and
  setters
...
int RigidBody::init(int sz, boost::python::list masses,
  ...
  */
  ...
  for (int i = 0; i < sz1; i++) {
    tmp[i] = boost::python::extract<double>(
        masses[i]); //    cout<<"mass "<<i<<"  = "<<tmp[i]<<endl;
    vtmp[i] = boost::python::extract<VECTOR>(
        positions[i]); //    cout<<"v "<<i<<"  = "<<vtmp[i]<<endl;
  }
...
int RigidBody::set_position(const VECTOR &cm) {
  /**
    \brief Set the center of mass of the RB, no matter what the masses and
    positions of the material points
...
int RigidBody::set_momentum(const VECTOR &p) {
  /**
    \brief Set the momentum of the RB, no matter what are the properties of the
    material points
...
int RigidBody::set_velocity(const VECTOR &v) {
  /**
    \brief Set the velocity of the RB, no matter what are the properties of the
    material points
...
int RigidBody::set_force(const VECTOR &f) {
  /**
    \brief Set the force of the RB, no matter what are the properties of the
    material points
...
int RigidBody::set_torque(const VECTOR &t) {
  /**
    \brief Set the torque (in internal frame) of the RB, no matter what are the
    properties of the material points
...
int RigidBody::set_forces_and_torques(int sz, VECTOR *r, VECTOR *f) {
  /**
    ...
    \param[in] r Coordinates (in body frame) of the centers to which the forces
    (Cartesian) are applied \param[in] f External forces (in lab frame) acting
    on the rigid body
  */
  rb_force = 0.0;
  is_rb_force = 1;
...
int RigidBody::set_inertia(const MATRIX3x3 &Ie) {
  /**
    \brief Set intertia matrix, no matter what are the properties of the
    material points
...
int RigidBody::set_orientation(const MATRIX3x3 &at) {
  /**
    ...
    In addition, the transposed transformation matrix and the quaternion will be
    updated
  ...
  */
  ...
  rb_A_I_to_e = at;
  is_rb_A_I_to_e = 1;
  rb_A_I_to_e_T = rb_A_I_to_e.T();
  is_rb_A_I_to_e_T = 1;
  MATRIX_TO_QUATERNION(rb_A_I_to_e, rb_L);
  is_rb_L = 1;
  return 0;
}

int RigidBody::set_orientation(const VECTOR &u1, const VECTOR &u2,
                               const VECTOR &u3) {
  /**
    \brief Set orientation given by 3 orientation vectors (directions of the
    body frame coordinate system) This is just a different interface to input
    the transformation matrix: Each vector is just a column of the 3x3
    transformation matrix. In addition, the transposed transformation matrix and
    the quaternion will be updated
...
int RigidBody::set_orientation(const QUATERNION &q) {
  /**
    ...
    In addition, the moving/body frame transformation matrix and its transpose
    will be updated

    \param[in] q The orientation quaternion. The first comonent gives the
    rotation angle around the axis, and the last 3 components define such axis
  */
  rb_L.init(q);
  is_rb_L = 1;
  QUATERNION_TO_MATRIX(rb_L, rb_A_I_to_e);
  is_rb_A_I_to_e = 1;
  rb_A_I_to_e_T = rb_A_I_to_e.T();
  is_rb_A_I_to_e_T = 1;
  return 0;
...
void RigidBody::init_S_matrix(MATRIX &S) {
  /**
    ...
    This matrix is needed for convenient execution of quaternion multiplication
    - in a standard matrix-vector form

    \param[out] S The matrix representation of the oerator that multiplies a
    vector by the quaternion
  */
  S.M[0] = rb_L.Lt;
  S.M[1] = -rb_L.Lx;
  S.M[2] = -rb_L.Ly;
...
int RigidBody::set_angular_momentum(const VECTOR &l) {
  /**
    ...
    If the rotational constants, rb_A , rb_B , and rb_C are defined, the angular
    velocities will be updated If the rotational constants are not defined, but
    the inverse of the inertia tensor matrix is deined, the latter will be used
    to compute angular velocities The quaternion conjugate momentum will be also
    updated. This is needed, if the quaternion-based integration schemes are
    used together with angular momentum.
  ...
  */
  ...
  if (is_rb_A && is_rb_B && is_rb_C) {
    rb_w_e.x = rb_A * rb_l_e.x;
    rb_w_e.y = rb_B * rb_l_e.y;
    rb_w_e.z = rb_C * rb_l_e.z;
    is_rb_w_e = 1;
  } else if (is_rb_invI_e) {
    rb_w_e = rb_invI_e * rb_l_e;
    is_rb_w_e = 1;
  }
...
int RigidBody::set_angular_momentum(const double &lx, const double &ly,
                                    const double &lz) {
  /**
...
int RigidBody::set_angular_velocity(const VECTOR &w) {
  /**
    ...
    The quaternion conjugate momentum will be also updated. This is needed, if
    the quaternion-based integration schemes are used together with angular
    momentum.
...
int RigidBody::set_angular_velocity(const double &wx, const double &wy,
                                    const double &wz) {
  /**
...
int RigidBody::set_quaternion_momentum(const QUATERNION &qm) {
  ...
  */
  ...
  rb_l_e.init(tmp.Lx, tmp.Ly, tmp.Lz);
  is_rb_l_e = 1;
  rb_w_e.init(rb_A * rb_l_e.x, rb_B * rb_l_e.y, rb_C * rb_l_e.z);
  is_rb_w_e = 1;
