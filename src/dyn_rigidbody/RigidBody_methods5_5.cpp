...
/**
  ...
  \brief The file implements the Terec and qTerec propagation method based on
  recursive Taylor series expansion

  See more in:
  Akimov, A. V.; Kolomeisky, A. B. Recursive Taylor Series Expansion Method for
  Rigid-Body Molecular Dynamics. J. Chem. Theory Comput. 2011, 7, 3062–3071.
...
void RigidBody::initialize_terec(int series_expansion) {
  /**
    \brief This is an auxiliary function that initializes auxiliary variables
    and parameters \param[in] series_expansion The length of the Taylor
    expansion chosen for the integration
  */
...
void RigidBody::propagate_terec(double t) {
  /**
    ...
    problem. See more in:
    Akimov, A. V.; Kolomeisky, A. B. Recursive Taylor Series Expansion Method
    for Rigid-Body Molecular Dynamics. J. Chem. Theory Comput. 2011, 7,
    3062–3071.
  ...
  */
  ...
  for (n = 2; n < SERIES_EXPANSION; n++) {
    for (int k = 0; k < n; k++) {
      ...
      u1[n].x += bin * (rb_C * deriv[k].z * u1[n - k - 1].y -
                        rb_B * deriv[k].y * u1[n - k - 1].z);
      u1[n].y += bin * (rb_A * deriv[k].x * u1[n - k - 1].z -
                        rb_C * deriv[k].z * u1[n - k - 1].x);
      u1[n].z += bin * (rb_B * deriv[k].y * u1[n - k - 1].x -
                        rb_A * deriv[k].x * u1[n - k - 1].y);

      u2[n].x += bin * (rb_C * deriv[k].z * u2[n - k - 1].y -
                        rb_B * deriv[k].y * u2[n - k - 1].z);
      u2[n].y += bin * (rb_A * deriv[k].x * u2[n - k - 1].z -
                        rb_C * deriv[k].z * u2[n - k - 1].x);
      u2[n].z += bin * (rb_B * deriv[k].y * u2[n - k - 1].x -
                        rb_A * deriv[k].x * u2[n - k - 1].y);

      u3[n].x += bin * (rb_C * deriv[k].z * u3[n - k - 1].y -
                        rb_B * deriv[k].y * u3[n - k - 1].z);
      u3[n].y += bin * (rb_A * deriv[k].x * u3[n - k - 1].z -
                        rb_C * deriv[k].z * u3[n - k - 1].x);
      u3[n].z += bin * (rb_B * deriv[k].y * u3[n - k - 1].x -
                        rb_A * deriv[k].x * u3[n - k - 1].y);
  ...
  */
  ...
  for (n = 2; n < SERIES_EXPANSION; n++) {
    for (int k = 0; k < n; k++) {
      ...
      derivq[n].Lt += bin * 0.5 *
                      (-derivq[k].Lx * deriv[n - k - 1].x * rb_A -
                       derivq[k].Ly * deriv[n - k - 1].y * rb_B -
                       derivq[k].Lz * deriv[n - k - 1].z * rb_C);
      derivq[n].Lx += bin * 0.5 *
                      (derivq[k].Lt * deriv[n - k - 1].x * rb_A -
                       derivq[k].Lz * deriv[n - k - 1].y * rb_B +
                       derivq[k].Ly * deriv[n - k - 1].z * rb_C);
      derivq[n].Ly += bin * 0.5 *
                      (derivq[k].Lz * deriv[n - k - 1].x * rb_A +
                       derivq[k].Lt * deriv[n - k - 1].y * rb_B -
                       derivq[k].Lx * deriv[n - k - 1].z * rb_C);
      derivq[n].Lz += bin * 0.5 *
                      (-derivq[k].Ly * deriv[n - k - 1].x * rb_A +
                       derivq[k].Lx * deriv[n - k - 1].y * rb_B +
                       derivq[k].Lt * deriv[n - k - 1].z * rb_C);
