...
void RigidBody::propagate_omelyan(double t) {
  ...
  */
  ...
  // ----- Leap-frog-like iteration procedure to obtain angular velocities on
  // half step--------
  do {
    omega1.x =
        omega_e.x +
        rb_A * t *
            (rb_torque_e.x +
             0.5 * (Jy - Jz) * (omega_e.y * omega_e.z + omega0.y * omega0.z));
    omega1.y =
        omega_e.y +
        rb_B * t *
            (rb_torque_e.y +
             0.5 * (Jz - Jx) * (omega_e.x * omega_e.z + omega0.x * omega0.z));
    omega1.z =
        omega_e.z +
        rb_C * t *
            (rb_torque_e.z +
             0.5 * (Jx - Jy) * (omega_e.x * omega_e.y + omega0.x * omega0.y));
