...
public:
  vector<VECTOR> rb_centers;
  int rb_centers_size;

  //------ Basic dynamical variables and constants --------
  // Total mass
  double rb_mass;
  int is_rb_mass; ///< the mass of the RB and the flag showing its status
                  ///< (defined or not)
  double rb_iM;
  int is_rb_iM; ///< the inverse mass of the RB and the flag showing its status
                ///< (defined or not)

  // Center of mass
  VECTOR rb_cm;
  int is_rb_cm; ///< the position of the center of mass of the RB and the flag
                ///< showing its status (defined or not)

  // Linear momentum and velocity of the center of mass
  VECTOR rb_p;
  int is_rb_p; ///< the RB momentum and the flag showing its status (defined or
               ///< not)
  VECTOR rb_v;
  int is_rb_v; ///< the RB velocity and the flag showing its status (defined or
               ///< not)

  // Total force acting on the center of mass
  VECTOR rb_force;
  int is_rb_force; ///< the RB force and the flag showing its status (defined or
                   ///< not)

  // Inertia moments
  MATRIX3x3 rb_I_I;
  int is_rb_I_I; ///< the RB intertia tensor in movable coordinate system and
                 ///< the flag showing its status (defined or not)
  MATRIX3x3 rb_I_e;
  int is_rb_I_e; ///< the RB intertia tensor (diagonal) in internal coordinate
                 ///< system and the flag showing its status (defined or not)

  // Inverse inertia moments
  MATRIX3x3 rb_invI_I;
  int is_rb_invI_I; ///< the inverse RB intertia tensor in movable coordinate
                    ///< system and the flag showing its status (defined or not)
  MATRIX3x3 rb_invI_e;
  int is_rb_invI_e; ///< the inverse RB intertia tensor (diagonal) in internal
                    ///< coordinate system and the flag showing its status
                    ///< (defined or not)

  // Rotational constants
  double rb_A;
  int is_rb_A; ///< the RB rotational constant the flag showing its status
               ///< (defined or not)
  double rb_B;
  int is_rb_B; ///< the RB rotational constant the flag showing its status
               ///< (defined or not)
  double rb_C;
  int is_rb_C; ///< the RB rotational constant the flag showing its status
               ///< (defined or not)

  //  Attitude matrix and its transpose
  MATRIX3x3 rb_A_I_to_e;
  int is_rb_A_I_to_e; ///< the RB transformation matrix: from the movable to
                      ///< internal and the flag showing its status (defined or
                      ///< not) this is essentially the orientation variable
                      ///< (matrix)
  MATRIX3x3 rb_A_I_to_e_T;
  int is_rb_A_I_to_e_T; ///< the transpose of the RB transformation matrix: from
                        ///< the movable to internal and the flag showing its
                        ///< status (defined or not) this is essentially inverse
                        ///< of the rb_A_I_to_e matrix (inverse rotation)

  // Orientation quaternion and its conjugate momentum
  QUATERNION rb_L;
  int is_rb_L; ///< The orientation variables in the quaternion form and the the
               ///< flag showing its status (defined or not)
  QUATERNION rb_p_r;
  int is_rb_p_r; ///< The momentum conjugate to quaternion variables and the
                 ///< flag showing its status (defined or not)

  // Angular momentum and angular velocity (in body frame)
  VECTOR rb_l_e;
  int is_rb_l_e; ///< The angular momentum in the body frame and the the flag
                 ///< showing its status (defined or not)
  VECTOR rb_w_e;
  int is_rb_w_e; ///< The angular velocity in the body frame and the the flag
                 ///< showing its status (defined or not)

  // Total torque (in body-fixed coordinate system)
  VECTOR rb_torque_e;
  int is_rb_torque_e; ///< The torque (angular generalized force) in the body
                      ///< frame and the the flag showing its status (defined or
                      ///< not)

  // Fixation (constraint) flags
  int is_fixed_translation; ///< The flag showing whether the translation of
                            ///< this RB is forbidden (if = 1) or not (if = 0)
  int is_fixed_rotation; ///< The flag showing whether the rotation of this RB
                         ///< is forbidden (if = 1) or not (if = 0)

  // Orientation update option - controls how set_orientation function works
  int set_orientation_option; ///< if 1 - use the previous approach that is we
                              ///< convert the updated orientation matrix to the
                              ///< quaternions for the later use if 0 - forget
                              ///< about updating quaternions
...
private:
  ...
  // Constants
  double MACHPREC; ///< "Machine precision". Defined to 1e-15 during the
                   ///< initialization.
  ...
  double BIG; ///< When to set rotational constants to zero. If they are larger
              ///< than this: 1e+10
  double
      MAX_NO; ///< Stopping criteria for Jacobi orthogonalization method: 1e-15
  double minDet; ///< Minimal value of determinant to treat matrix as
                 ///< non-degenerate: 1e-10

  MATRIX3x3 U[7]; ///< Permutation matrices
  int permutindx; ///< Index of permutation matrix necessary for Jacobi ordering
  int invpermutindx; ///< Index of permutation matrix inverse for that with
                     ///< permutindx;
  int orderflag;
  int SERIES_EXPANSION; ///< For TEREC algorithm, the number of terms in
  ...
  double eps;
  double K, Kcompl; ///< quater period and complimentary quater period K =
                    ///< F(1|m), Kcompl = F(1|1-m)
