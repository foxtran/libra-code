...
/**
  ...
  setters So far these methods are private, so they are not reflected in the
...
void RigidBody::calc_mass(int sz, double *m) {
  ...
  */
  ...
  rb_iM = (rb_mass == 0.0) ? 0.0 : (1.0 / rb_mass);
  is_rb_iM = 1;
}
...
void RigidBody::calc_orientations(int sz, double *m, VECTOR *r) {
  ...
  rb_A_I_to_e_T.xx = C.get(0, 0);
  rb_A_I_to_e_T.xy = C.get(0, 1);
  rb_A_I_to_e_T.xz = C.get(0, 2);
  rb_A_I_to_e_T.yx = C.get(1, 0);
  rb_A_I_to_e_T.yy = C.get(1, 1);
  rb_A_I_to_e_T.yz = C.get(1, 2);
  rb_A_I_to_e_T.zx = C.get(2, 0);
  rb_A_I_to_e_T.zy = C.get(2, 1);
  rb_A_I_to_e_T.zz = C.get(2, 2);
...
void RigidBody::calc_inverse_tensors(int sz) {
  ...
  if (sz > 1) {
    if (fabs(rb_I_e.Determinant()) > minDet) {
      ...
      if (rb_I_e.xx > IEPS) {
        rb_invI_e.xx = 1.0 / rb_I_e.xx;
      } else {
        rb_invI_e.xx = 0.0;
      }

      if (rb_I_e.yy > IEPS) {
        rb_invI_e.yy = 1.0 / rb_I_e.yy;
      } else {
        rb_invI_e.yy = 0.0;
      }

      if (rb_I_e.zz > IEPS) {
        rb_invI_e.zz = 1.0 / rb_I_e.zz;
      } else {
        rb_invI_e.zz = 0.0;
      }
  ...
  if (fabs(rb_invI_e.xx) > BIG) {
    cout << "Warning: The fragment most likely was linear or otherwise "
            "degenerate\n";
  ...
  // Make it diagonal
  rb_invI_e.xy = rb_invI_e.xz = rb_invI_e.yx = rb_invI_e.yz = rb_invI_e.zx =
      rb_invI_e.zy = 0.0;
  is_rb_invI_e = 1;
