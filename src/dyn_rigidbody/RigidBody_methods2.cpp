...
/**
  ...
  \brief The file implements RigidBody class interface - modifiers, internal
  variables transformations/propagation
...
int RigidBody::scale_angular_(double sc) {
  /**
    ...
    This will scale: angular momentum (body frame), angular velocity (body
    frame), quaternion momentum
...
int RigidBody::scale_angular_(double scx, double scy, double scz) {
  /**
    ...
    This will first scale: angular momentum (body frame) and angular velocity
    (body frame) In this case all is more complicated rb_p_r = 2*S(q)*rb_l_e, so
    rb_p_r is calculated directly using scaled rb_l_e variable

    \param[in] scx Scaling parameter for x component of angular velocity and
    angular momentum \param[in] scy Scaling parameter for y component of angular
    velocity and angular momentum \param[in] scz Scaling parameter for z
    component of angular velocity and angular momentum
  */
...
int RigidBody::scale_angular_(const MATRIX3x3 &sc) {
  /**
    ...
    We will first transorm angular momentum and velocity (matrix-vector
    multiplication) In this case all is more complicated rb_p_r = 2*S(q)*rb_l_e,
    so rb_p_r is calculated directly using scaled rb_l_e variable

    \param[in] sc Is the transformation (generalized scaling, but actually more
    than just scaling) matrix.
  */
...
int RigidBody::scale_linear_(double scx, double scy, double scz) {
  /**
    ...
    \param[in] scx Scaling parameter for x component of linear velocity and
    linear momentum \param[in] scy Scaling parameter for y component of linear
    velocity and linear momentum \param[in] scz Scaling parameter for z
    component of linear velocity and linear momentum
  */
...
int RigidBody::scale_linear_(const MATRIX3x3 &sc) {
  /**
    ...
    We will transorm linear momentum and linear velocity (matrix-vector
    multiplication)
...
int RigidBody::apply_torque(double dt) {
  /**
    \brief Apply torque (the one already set in the RB object), to evolve
    angular momentum

    Angular momentum is in body frame. Quaternion momentum is also evolved. The
    angular velocity is not evolved.
...
int RigidBody::apply_force(double dt) {
  /**
    \brief Apply force (the one already set in the RB object), to evolve linear
    momentum
...
int RigidBody::apply_force(MATRIX3x3 &x) {
  /**
    ...
    \param[in] x The scaling factor that is applied to force before it is
    applied to evolve linear momentum
  */
