...
/**
  ...
  \brief The file implements the exact propagation method for free RB, based on
  Euler elliptic integrals
  ...
  if (L2 > 0.0) {
    ...
    //================= General case =================================
      ...
      //------------------ Symmetric top ----------------------------------
        ...
        if (!status) {
          cout << "In symmetric top case. The necessary permutation can not be "
                  "found"
               << endl;
          exit(1);
        } else {
      ...
      //----------------- Asymmetric top ---------------------------------
        ...
        for (int permut = 0; permut < 6; permut++) {
          ...
          I_tmp = U[permut] * rb_I_e *
                  U[invpermut]; // cout<<"permut = "<<permut<<"I_tmp =
                                // "<<I_tmp<<endl;
          Ix = I_tmp.xx;
          Iy = I_tmp.yy;
          Iz = I_tmp.zz;
        ...
        if (!status) {
          cout << "In asymmetric top case. The necessary permutation can not "
                  "be found"
               << endl;
          exit(1);
        } else {
        ...
        double omega2m = -copysign(
            sqrt(omega2 * omega2 + I1 * I13 * omega1 * omega1 / I2 / I23),
            omega1);
        double omega3m = copysign(sqrt(-L23 / I3 / I13), omega3);
        double omegap =
            I23 * copysign(sqrt(L23 / (-I23) / I1 / I2 / I3), omega3);
        ...
        wp = SIGN(I2 - I3) * sgnz *
             sqrt((L2 - 2.0 * I1 * Er) * (I3 - I2) / (I1 * I2 * I3));
        wp = omegap;

        // Elliptic parameter
        m = (L2 - 2.0 * I3 * Er) * (I1 - I2) /
            ((L2 - 2.0 * I1 * Er) * (I3 - I2));
        ...
        // ...instead of:
        // Kcompl = Km((1.0-m),1e-12);
        // //boost::math::ellint_1<double>(sqrt(1.0-m));
        q = exp(-M_PI * Kcompl / K); // nome
        // eta    = SIGN(top_w.z)*Kcompl -
        // boost::math::ellint_1<double,double>(sqrt(1.0-m),asin(I3*top_wm.z/L));
        // //Ellipe(asin(I3*top_wm.z/L),sqrt(1-m),IntN); f =
        // boost::math::ellint_1<double,double>(sqrt(1.0-m),asin(I3*top_wm.z/L));
        ...
        base =
            2.0 *
            sqrt(sqrt(1.0)); // q - according to formula, 1 - according to code
        double q_n = 1.0;
        double q_2n = 1.0;
        double q_n_2 = 1.0;
        ...
        for (n = 0; n < NT; n++) {
          cr[n] =
              base * q_n * q_n_2 * cosh(0.5 * (2.0 * n + 1.0) * M_PI * eta / K);
          ci[n] = -base * q_n * q_n_2 *
                  sinh(0.5 * (2.0 * n + 1.0) * M_PI * eta / K);
...
void RigidBody::propagate_exact_rb(double dt) {
  ...
  */
  ...
  double Lp;
  Lp = sqrt(top_l.x * top_l.x + top_l.y * top_l.y);
  ...
  if (Lmod > 0.0) {
    ...
    //================== General case ==========================
      ...
      //----------------- Symmetric top case ---------------------
        ...
        Rz = U[permutindx] * Rz *
             U[permutindx]; // This accounts for intermediate permutations
                            // of vector l_e
        P = T1 * T2;
        P = U[permutindx] * P *
            U[permutindx]; // This accounts for intermediate permutations
      ...
      //------------------- Asymmetric top case ------------------------
        ...
        u = wp * t + eps;
        Km_ = Km(m,
                 1e-12); // boost::math::ellint_1<double>(sqrt(m)); //Km(m,tol);
        for (int n = 0; n < NT; n++) {
