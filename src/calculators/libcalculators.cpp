...
void export_calculators_objects() {
  ...
  */
  ...
  double (*expt_fermi_population_v1)(double e, double ef, double degen,
                                     double kT) = &fermi_population;

  double (*expt_fermi_integral_v1)(vector<double> & bnds, double ef,
                                   double degen, double kT) = &fermi_integral;

  double (*expt_fermi_integral_v2)(boost::python::list bnds, double ef,
                                   double degen, double kT) = &fermi_integral;

  double (*expt_fermi_energy_v1)(vector<double> & bnds, double Nel,
                                 double degen, double kT, double etol) =
      &fermi_energy;

  double (*expt_fermi_energy_v2)(boost::python::list bnds, double Nel,
                                 double degen, double kT, double etol) =
      &fermi_energy;
  ...
  boost::python::list (*expt_populate_bands_v1)(
      double Nel, double degen, double kT, double etol, int pop_opt,
      boost::python::list bands) = &populate_bands;

  def("order_bands", expt_order_bands_v1);
  def("populate_bands", expt_populate_bands_v1);

  //----------------- Density_Matrix.cpp ------------------------
  MATRIX(*expt_compute_density_matrix_v1)
  (boost::python::list occ, MATRIX C) = &compute_density_matrix;
  CMATRIX(*expt_compute_density_matrix_v2)
  (boost::python::list occ, CMATRIX C) = &compute_density_matrix;

  boost::python::list (*expt_Fock_to_P_v1)(MATRIX Fao, MATRIX Sao, double Nel,
                                           double degen, double kT, double etol,
                                           int pop_opt) = &Fock_to_P;
  boost::python::list (*expt_Fock_to_P_v2)(CMATRIX Fao, CMATRIX Sao, double Nel,
                                           double degen, double kT, double etol,
                                           int pop_opt) = &Fock_to_P;
  ...
  //----------------- Excitations.cpp ---------------------------
  boost::python::list (*expt_excite_v1)(int I, int J,
                                        boost::python::list occ_ini) = &excite;

  def("excite", expt_excite_v1);

  //----------------- Energy_Electronic.cpp ---------------------
  double (*expt_energy_elec_v1)(MATRIX Pao, MATRIX Hao, MATRIX Fao) =
      &energy_elec;
  double (*expt_energy_elec_v2)(MATRIX P_alp, MATRIX P_bet, MATRIX Hao_alp,
                                ...
                                MATRIX temp) = &energy_elec;

  def("energy_elec", expt_energy_elec_v1);
  def("energy_elec", expt_energy_elec_v2);

  //----------------- Energy_Nuclear.cpp ---------------------
  double (*expt_energy_nucl_v1)(vector<VECTOR> & R, vector<double> & Zeff) =
      &energy_nucl;
  double (*expt_energy_nucl_v2)(vector<VECTOR> & R, vector<double> & Zeff,
                                vector<VECTOR> & G) = &energy_nucl;

  def("energy_nucl", expt_energy_nucl_v1);
  def("energy_nucl", expt_energy_nucl_v2);

  //----------------- Mulliken.cpp ---------------------------------
  boost::python::list (*expt_update_Mull_orb_pop_v1)(MATRIX P, MATRIX S) =
      &update_Mull_orb_pop;
