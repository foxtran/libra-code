...
/**
  ...
  \brief The file implements functions for creation of the excitation objects
...
void excite(int I, int J, vector<pair<int, double>> &occ_ini,
            vector<pair<int, double>> &occ_fin) {
  /**
    \brief Create a list representing a I --> J ecxitation

    basically, vector< pair<int,double> > datastructure represents Slater
    determinant (by selecting ordering of orbitals in the active space)
...
boost::python::list excite(int I, int J, boost::python::list occ_ini) {
  /**
    ...
    basically, vector< pair<int,double> > datastructure represents Slater
    determinant (by selecting ordering of orbitals in the active space) Returns
    the final occupation list
...
void excite(int Norb, excitation &ex, int Nocc_alp,
            vector<pair<int, double>> &occ_alp, int Nocc_bet,
  ...
  // ...
  // from[N] -> to[N]
  // Example: from = [ 0A, 0B] to = [1A, 1B] - double excitation of 0A -> 1A &
  // 0B -> 1B Here 0 = HOMO, -1 = HOMO-1, 1 = LUMO, 2 = LUMO+1, etc.

  // This function annihilates on <from> whatever is there and adds it to <to>
  // whatever exists there already This function changes occ_alp and occ_bet -
  // these are both input and output variables
  ...
  for (int i = 0; i < ex.size; i++) {
    ...
    } else { // In this case there is more that can be take from the source than
             // can be put to the target
...
void compute_excitations(Control_Parameters& prms,Model_Parameters&
modprms,Nuclear& mol, vector<int>& fragment, vector<int>& basis_fo,vector<AO>&
basis_ao,vector<vector<int> >& at_orbitals, Electronic* el,Electronic* el0,
Memory* mem){

//
  int Norb = el_tmp->Norb;
  double E_ground =
::energy_elec(Norb,el_tmp->P_alp,el_tmp->Hao,el_tmp->Fao_alp)
                  +
::energy_elec(Norb,el_tmp->P_bet,el_tmp->Hao,el_tmp->Fao_bet); cout<<"E_ground =
"<<E_ground<<endl;

  // Now compute vertical excitation energies
  vector< pair<int,double> > occ_alp_grnd(Norb,pair<int,double>(0,0.0));
occ_alp_grnd = el_tmp->occ_alp; vector< pair<int,double> >
occ_bet_grnd(Norb,pair<int,double>(0,0.0)); occ_bet_grnd = el_tmp->occ_bet;
  vector< pair<int,double> > docc(Norb,pair<int,double>(0,0.0));


  // Important!!! Do not recompute density from MO populations - because the
final density may in fact be
  // an ensemble density (of last two few iterates - e.g. if DIIS or damping is
used)
  // So we only compute changes of density matrix that correspond to change of
MO populations within reasonable limits MATRIX* dP_;    dP_ = new
MATRIX(Norb,Norb); *dP_ = 0.0;
  ...
  for(e=0;e<prms.num_excitations;e++){
    ...
    excite(Norb, prms.excitations[e], el_tmp->Nocc_alp, el_tmp->occ_alp,
el_tmp->Nocc_bet, el_tmp->occ_bet); // ground state excitation


    cout<<"Excitation: "<<prms.excitations[e].from_orbit[0]<<"
"<<prms.excitations[e].from_spin[0]<<"  -->  "
                        <<prms.excitations[e].to_orbit[0]<<"
"<<prms.excitations[e].to_spin[0]<<endl;


    // Update delta of density matrix
    //!!! But keep in mind that WFC is not always equivalent to density matrix
    for(i=0;i<Norb;i++){ docc[i] = pair<int,double>(i, el_tmp->occ_alp[i].second
- occ_alp_grnd[i].second); } compute_density_matrix(docc, el_tmp->C_alp, dP_);
    *el_tmp->P_alp = *el->P_alp  +  *dP_;

    for(i=0;i<Norb;i++){ docc[i] = pair<int,double>(i, el_tmp->occ_bet[i].second
- occ_bet_grnd[i].second); } compute_density_matrix(docc, el_tmp->C_bet, dP_);
    *el_tmp->P_bet = *el->P_bet + *dP_;

    *el_tmp->P = *el_tmp->P_alp + *el_tmp->P_bet;  // this is an excited state
density matrix (up to our approximation)


    // Update Fock matrix
    Hamiltonian_Fock(prms,modprms,mol_tmp, fragment,
basis_fo,basis_ao,at_orbitals,el_tmp,el0,mem); // el - now contains an updated
Fock matrix
...
::energy_elec(Norb,el_tmp->P_bet,el_tmp->Hao,el_tmp->Fao_bet);
    ...
    // Alp component
    ii1 = el_tmp->Nocc_alp + prms.excitations[e].from_orbit[0] - 1; // index of
the source MO ii2 = el_tmp->Nocc_alp + prms.excitations[e].to_orbit[0] - 1;   //
index of the target MO
    ...
    cout<<"Excitation a= "<<e<<" E-E0,eV= "<<prms.excitations[e].Energy/eV<<"
oscillator strength= "<<f<<" Tr(S*D)= "<<(*el_tmp->Sao * *el_tmp->P).tr()<<endl;
