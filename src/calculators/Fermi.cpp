...
double fermi_integral(std::vector<double> &bnds, double ef, double degen,
                      double kT) {
  /**
...
double fermi_integral(boost::python::list bnds, double ef, double degen,
                      double kT) {
  /**
...
double fermi_energy(std::vector<double> &bnds, double Nel, double degen,
                    double kT, double etol) {
  /**
  ...
  */
  ...
  do {
    ...
    if (0) { // For debug
      cout << "ef_l= " << ef_l << " ef_m= " << ef_m << " ef_r= " << ef_r
           << endl;
      cout << "i_l= " << i_l << " i_m= " << i_m << " i_r= " << i_r << endl;
    }

    int var;
    if (i_m * i_r <= 0.0 && i_m * i_l >= 0.0) {
      var = 1;
    } else if (i_m * i_r >= 0.0 && i_m * i_l <= 0.0) {
      var = 2;
    } else {
      cout << "Error in fermi_energy\n";
      exit(0);
    }
...
double fermi_energy(boost::python::list bnds, double Nel, double degen,
                    double kT, double etol) {
  /**
...
double p_ef(double e, double ef, double de) {
  ...
  if (argg < -100) {
    ...
    res = 1.0 / (1.0 + exp(argg));
  }
  return res;
}

void Chebyshev_coeff(vector<double> &C,
                     double (*f)(double x, double y, double z), double ef,
                     double de, int N) {
  ...
  // double (*f)(double x, double y, double z) -is a scalar function which we
  are approximating
  // ef - trial parameter (Fermi energy, upper or lower eigenvalue)
  // de - energy width parameter (smaller it is, the higher the accuracy, but is
  slower calculation)
  ...
  */
  ...
  //  for(int k=0;k<=np;k++){  cout<<"k= "<<k<<"  C["<<k<<"]= "<<C[k]<<endl; }//
  //  for k
}

double Chebyshev_fit(MATRIX &H, MATRIX &P,
                     double (*f)(double _x, double _y, double _z), double ef,
                     double de, int np) {
  /**
  // Compute density matrix using Chebyshev polynomials
  // Return trace of the density matrix, for it should be equal to the number of
  electrons
  */
  ...
  vector<double> c_k(np + 1, 0.0);
  Chebyshev_coeff(c_k, f, ef, de, np);
  ...
  for (int k = 0; k <= np; k++) {
    ...
    if (k == 0) {
      P -= 0.5 * c_k[0] * T_k;
    } // T_k is simply unity matrix in this case
